...
- 可按工具或类型筛选，也可直接查看最近复制和收藏
- 首次打开可选择常用工具；右上角「管理」可切换平台、隐藏或恢复工具
...
- **更新已有**：点击对应工具的「检查更新」，先查看新增、修改、删除和元数据差异，再决定应用或放弃
...
1. 点插件图标打开面板
2. 点击右上角「管理」
3. 新增时输入工具名并点击「查询并新增」；更新时点击对应工具的「检查更新」
4. 更新完成分析后检查差异摘要，选择「应用更新」或「放弃」
5. 等待1-3分钟（后台在调用 Claude Code 查文档并返回结构化数据，过程中可以关闭面板）
6. 只有确认应用且数据确实变化时，插件才会重新加载
...
插件本身不包含任何网络请求代码，不会上传、收集或分享任何数据。本机浏览器会保存搜索词、收藏、最近 20 条复制记录、平台和启用工具偏好。
