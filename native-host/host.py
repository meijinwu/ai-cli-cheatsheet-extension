...
SHELL_BATCHES = [
    {"id": "posix-core", "label": "sh/POSIX 基础", "topics": ["builtins", "syntax"], "scope": "POSIX sh 内置命令与基础语法：cd、pwd、echo、test/[、export、set、unset、:（空操作）、.（source）、退出码 $?"},
    {"id": "bash-builtins", "label": "bash 内置与特性", "topics": ["builtins"], "scope": "bash 内置命令与特性：type、command、alias、declare、local、shopt、read、printf、bind 等（只收录 bash 解释器本体）"},
    {"id": "zsh-builtins", "label": "zsh 内置与特性", "topics": ["builtins"], "scope": "zsh 内置命令与特性：autoload、setopt、zstyle、whence、以及与 bash 的差异（只收录 zsh 解释器本体）"},
    {"id": "syntax-keywords", "label": "语法与关键字", "topics": ["syntax", "scripting"], "scope": "关键字 if/for/while/case/function；管道 |、重定向 > >> <、通配符 *、变量 $VAR、命令替换 $(...)、引用与转义"},
    {"id": "shortcuts", "label": "终端快捷键", "topics": ["shortcuts"], "scope": "命令行行编辑快捷键（emacs 键位）：Ctrl+A、Ctrl+E、Ctrl+R、Ctrl+U、Ctrl+K、Ctrl+W、Tab 补全触发等"},
    {"id": "config-files", "label": "配置文件", "topics": ["config"], "scope": "~/.zshrc、~/.bashrc、~/.zprofile、~/.bash_profile 等：加载顺序、登录/交互式区别、何时用哪个"},
    {"id": "environment", "label": "环境变量与别名", "topics": ["environment"], "scope": "环境变量、PATH、export、alias、function、source 重新加载配置、env 查看变量"},
    {"id": "history-completion", "label": "历史与补全", "topics": ["history", "completion"], "scope": "history、历史展开 !!/!$、Ctrl+R 反向搜索、Tab 补全与补全系统配置"},
    {"id": "troubleshooting", "label": "命令来源与脚本排错", "topics": ["troubleshooting", "scripting"], "scope": "type -a、command -v、which 排查命令来源/别名遮蔽；set -e、set -u、set -o pipefail、$? 脚本排错"},
]
...
3. 覆盖本批次的 Shell 内置命令、关键字、语法、快捷键、配置项或排错点；不要包含极冷门历史兼容细节。
